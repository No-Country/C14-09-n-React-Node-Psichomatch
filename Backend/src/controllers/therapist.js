...
const createTherapist = async (req, res) => {
    try {
        ...
        if(!name || !lastName || !adress) return res.status(400).json({ error: "Missing fields" });
...
const updateTherapistPriceByPorcent = async (req, res) => {
    ...
    try {
        ...
        if(porcent < 0 || porcent > 100) return res.status(400).json({ error: "Porcent must be between 0 and 100" });
        const therapist = await Therapist.findByPk(id);
